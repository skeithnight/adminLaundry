...
class LoginActivity : AppCompatActivity(){
    ...
    private fun attemptLogin() {
        ...
        if (cancel) {
            ...
            administrator.username = username_loginStr
            administrator.password = password_loginStr
            showProgress(true)
//            val gson: Gson = Gson()
//            Toast.makeText(this@LoginActivity, gson.toJson(customer),Toast.LENGTH_LONG).show()
            getToken(administrator)
        }
    }
    ...
    private fun getToken(administrator: Administrator) {

        val service = APIClient.getClient().create(AuthService::class.java)

        service.postLogin(administrator).enqueue(object : Callback<ResponseLogin> {
